...
module Data.Vector.Generic.Mutable (
  ...
  unstablePartition, unstablePartitionBundle, partitionBundle, myFilterBundle
) where
...
myFilterBundle :: (PrimMonad m, MVector v a)
  => (a -> Bool) -> (a -> Bool) ->  Bundle u a -> m (v (PrimState m) a, v (PrimState m) a)
{-# INLINE myFilterBundle #-}
myFilterBundle p1 p2 s
  = case upperBound (Bundle.size s) of
      Just n  -> myFilterMax p1 p2 s n
      Nothing -> error "Can't help you"

myFilterMax :: (PrimMonad m, MVector v a)
  => (a -> Bool) -> (a -> Bool) -> Bundle u a -> Int -> m (v (PrimState m) a, v (PrimState m) a)
{-# INLINE myFilterMax #-}
myFilterMax p1 p2 s n
  = do
      v <- INTERNAL_CHECK(checkLength) "unstablemyFilterMax" n
         $ unsafeNew n
      let {-# INLINE_INNER put #-}
          put (i,j) x
            | p1 x       = if p2 x
      ...
      (i,j) <- Bundle.foldM' put (0,n) s
      INTERNAL_CHECK(check) "myFilterMax" "invalid indices" (i <= j)
        $ return ()
      let l = unsafeSlice 0 i v
          r = unsafeSlice j (n-j) v
      reverse r
...
myFilterUnknown :: (PrimMonad m, MVector v a)
  => (a -> Bool) -> (a -> Bool) ->  Bundle u a -> m (v (PrimState m) a, v (PrimState m) a)
{-# INLINE myFilterUnknown #-}
myFilterUnknown p1 p2 s =
  do
    v1 <- unsafeNew 0
    v2 <- unsafeNew 0
    (v1', n1, v2', n2) <- Bundle.foldM' put (v1, 0, v2, 0) s
