...
module Data.Vector.Generic (
  ...
  -- ** Partitioning
  partition, unstablePartition, span, break, myFilter,
...
myFilter :: Vector v a => (a -> Bool) -> (a -> Bool) -> v a -> (v a, v a)
{-# INLINE myFilter #-}
myFilter p1 p2 = myFilter_stream p1 p2 . stream

myFilter_stream :: Vector v a => (a -> Bool) -> (a -> Bool) -> Bundle u a -> (v a, v a)
{-# INLINE_FUSED myFilter_stream #-}
myFilter_stream p1 p2 s = s `seq` runST (
  do
    (mv1,mv2) <- M.myFilterBundle p1 p2 s
    v1 <- unsafeFreeze mv1
    v2 <- unsafeFreeze mv2
    return (v1,v2))
